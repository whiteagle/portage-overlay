...
pkg_config() {
	...
	einfo
	einfo "Continue with setting up /etc/syslog-ng/syslog-ng.conf rules for PostgreSQL?"
	read EINPUT_IANSWER
	...
	read -p "Detected a syslog-ng.conf in /etc/syslog-ng/, is this the one you want to use? (y/n): " EINPUT_IANSWER
		if [ "${EINPUT_IANSWER}" == "y" ] || [ "${EINPUT_IANSWER}" == "yes" ];then
			szSysLogConf="/etc/syslog-ng/syslog-ng.conf"
		else
			einfo "Please specify the path to your syslog-ng.conf" "/etc/syslog-ng/syslog-ng.conf"
			read 
		fi
	fi
	
	if [ ! -w ${szSysLogConf} ];then
	read -p "Cannot use ${szSysLogConf}, please specify the path to a valid syslog-ng.conf ( /etc/syslog-ng/syslog-ng.conf ):" EINPUT_IANSWER
	...
	einfo "syslog-ng filter?" "filter f_postgres { program(\"postgres\"); };"
	read EINPUT_IANSWER
	szSysLogFilter=${EINPUT_IANSWER}
	if [ ! -d /var/log/postgresql ];then
		echo "Specify where I should attempt to create the PostgreSQL log directory" "/var/log/postgresql"
		read EINPUT_IANSWER
	...
	einfo "syslog-ng destination?" "${szDefaultDest}"
	read EINPUT_IANSWER
	...
	einfo "syslog-ng rule?" "log { source(src); filter(f_postgres); destination(dest_postgres); };"
	read EINPUT_IANSWER
	...
	einfo "${szSysLogRule}"
	einfo 
	einfo "Continue with writing these rules to ${szSysLogConf}?"
	read EINPUT_IANSWER
		if [ "${EINPUT_IANSWER}" == "y" ] || [ "${EINPUT_IANSWER}" == "yes" ];then
	...
	einfo "Continue with setting up a daily cron job for reports?"
	read EINPUT_IANSWER
	if [ ${EINPUT_IANSWER} == "y" ] || [ ${EINPUT_IANSWER} == "yes" ];then
		einfo
