...
src_configure() {
	...
	MY_PY="/usr/bin/python$(python_get_version)"
...
src_install() {
	...
	# bug #166738
	$(python_get_version)
	mv "${D}"/usr/$(get_libdir)/python"$(python_get_version)"/site-packages/PortableServer.py \
		"${D}"/usr/$(get_libdir)/python"$(python_get_version)"/site-packages/omniorbpy_PortableServer.py

	mv "${D}"/usr/$(get_libdir)/python"$(python_get_version)"/site-packages/CORBA.py \
		"${D}"/usr/$(get_libdir)/python"$(python_get_version)"/site-packages/omniorbpy_CORBA.py

	rm "${D}"/usr/$(get_libdir)/python"$(python_get_version)"/site-packages/omniidl_be/__init__.py*

	# fixed the file collision from bug #247851
	rm "${D}"/usr/$(get_libdir)/python"$(python_get_version)"/site-packages/__init__.py
}

pkg_postinst() {
	#python_version
	python_mod_optimize /usr/$(get_libdir)/python"$(python_get_version)"/site-packages
}
